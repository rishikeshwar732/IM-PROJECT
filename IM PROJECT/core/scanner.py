...
def _detect_azure_nsg_findings(nsg: Dict[str, Any], file_path: str) -> List[Finding]:
    ...
    for idx, rule in enumerate(rules):
        ...
        dest_port = str(
            ...
            or props.get("destinationPortRanges")
            or ""
        )

        is_inbound_allow = direction == "inbound" and access == "allow"
        is_open_source = source in {"*", "0.0.0.0/0"}
        exposes_ssh = "22" in dest_port.split(",") or dest_port == "22"
...
def _infer_provider_and_service(file_path: str, policy: Dict[str, Any]) -> (str, str):
